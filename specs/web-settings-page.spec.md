...
- Touches:
  - `web` — the driving adapter only.
    Adds the `/settings` page (a real form bound to `GET`/`PUT /config`), a thin `useConfig` / `useUpdateConfig` query layer over the existing `apiFetch` + `QueryClient`, a Yup form schema (`config-schema.ts`) driven via `yupResolver`, the `sonner` toast surface, and a `Toaster` mounted in `AppShell`.
  - `core` — **no changes**.
    `parseConfig` / `Period` are the server-side authority, reused verbatim by the API.
  - `api` — **no changes**.
...
Form validation uses a **Yup** schema (`config-schema.ts`) via `yupResolver`, for label-aware per-field messages; `@lametrader/core`'s `parseConfig` remains the server-side authority (the API re-validates every write).
See `docs/decisions/0011-web-form-validation-with-yup.md` for the trade-off (UI-scoped schema vs. single-source).
...
- **Validation** → A Yup schema (`config-schema.ts`) via `yupResolver`, giving label-aware per-field messages. UI-scoped; `parseConfig` stays the server authority (ADR 0011).
...
- [ ] An unselected required option surfaces a label-aware inline error **on that field** (no periods → "Select at least one period." under the timeframe bar; cleared `defaultPeriod` → "Default period is required." under the select) and **disables Save**, so the invalid state can't be submitted (no `PUT`). Messages come from the Yup schema's `.label(...)` + per-rule copy.
...
Config schema (`configSchema` via `yupResolver`):

- [ ] A valid `{ periods, defaultPeriod }` resolves with no errors.
- [ ] An empty `periods` flags the `periods` field with "Select at least one period.".
- [ ] An empty `defaultPeriod` flags the `defaultPeriod` field with "Default period is required.".
- [ ] A `defaultPeriod` not among the selected periods flags the `defaultPeriod` field.
...
- **Validation evolved during review.** It started as a `parseConfig`-backed custom resolver (to keep one source of truth), but `parseConfig` can't produce label-aware per-field messages, and the workarounds (presence guards, error-`field` tagging, message string-rewriting) were each duplication or hacky.
  Landed on a UI-scoped Yup schema (`yupResolver`) for the form, with `parseConfig` kept as the server authority — see ADR 0011.
