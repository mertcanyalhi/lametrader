...
- [ ] An unselected required option surfaces an inline error **on that field** (no periods → "Select at least one period." under the timeframe bar; cleared `defaultPeriod` → "Select a default period." under the select) and **disables Save**, so the invalid state can't be submitted (no `PUT`). `parseConfig` decides validity and which field is at fault (its `field` tag); the resolver attaches that field's user-facing message — neither RHF nor `parseConfig` has a native label facility, so the presentation copy is authored in the resolver (no rule re-implemented, no domain-message string parsing).
