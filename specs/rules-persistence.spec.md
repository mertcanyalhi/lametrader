...
`.RuleRepository` is extended from the current minimal read/save surface to full CRUD parity with v1's repo:
...
`.EventLog` is unchanged from #393's port:
...
Lives at `packages/engine/src/rules/persistence/testing/`:
