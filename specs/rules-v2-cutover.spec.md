## Spec: rules-v2 cutover + v1 cleanup

- Status: draft
- Touches: `@lametrader/engine` (drop v1 `rules/`, keep `rules-v2/`), `@lametrader/core` (drop v1 rule types no longer referenced), `@lametrader/api` (drop `/rules` + `/symbols/:id/rule-events` v1 routes), `@lametrader/web` (drop v1 rules UI; promote v2 to `/rules`), `@lametrader/cli` (drop v1 `rules` command if any), docs (ADR 0012 status, ADR 0016 mention)

## Goal

Retire the v1 rules surface end-to-end now that v2 ships behind the flag (issue #396).
The cutover PR deletes v1 code, drops the `rulesV2` feature flag, and surfaces v2 as the only editor at `/rules`.
...
- [ ] No v1 modules remain under `packages/engine/src/rules/` (the directory itself is removed; v2 stays in `packages/engine/src/rules-v2/`).
...
- [ ] `connectServices()` returns `ConnectedServices` without a v1 `rules` or `wiredRuleEngine` field; only the v2 `rulesV2` and `wiredRuleEngineV2` survive.
- [ ] The API rejects `GET /rules` and `GET /symbols/:id/rule-events` with 404 (no route registered), and `GET /v2/rules` continues to return the v2 list (smoke).
- [ ] The web app no longer reads the `rulesV2` feature flag; the `/rules` route renders the v2 editor unconditionally; the `Rules v2` sidebar entry is gone (one `Rules` entry routes to the v2 editor).
- [ ] No file under `packages/web/src/pages/rules/` exists (the directory is removed); the v2 pages move from `packages/web/src/pages/rules-v2/` to `packages/web/src/pages/rules/` (so the URL and the source directory match again).
...
The happy path the e2e test asserts: a freshly built API rejects `GET /rules` with 404 (route not registered) and continues to serve `GET /v2/rules` (one new e2e — `rules-v1-cutover.e2e.test.ts` — added in this PR). The existing v2 e2e (`rules-v2.e2e.test.ts`) continues to pass against the same fixture. The web e2e (`rules-v2-ui.e2e.test.ts`) — renamed and updated to read `/rules` instead of `/rules-v2` — continues to pass without a feature flag override.
...
- Renaming v2 modules / symbols / files (e.g. `RuleServiceV2` → `RuleService`, `wire-rule-engine-v2.ts` → `wire-rule-engine.ts`). The acceptance criteria explicitly accept v2's `-V2` suffixes per the issue framing: "or keep -v2 suffixes if removing them creates a giant diff with no behavioural change."
...
- The v2 directory layout itself (still `packages/engine/src/rules-v2/`). Only the web URL + folder pair (`rules-v2/` → `rules/`) move so the URL no longer leaks the v2 origin.
