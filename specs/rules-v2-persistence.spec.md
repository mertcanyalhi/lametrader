# Spec: rules-v2 persistence (rules_v2 collection + Mongo/in-memory RuleRepository + EventLog)

- Status: draft
- Touches: `RulesV2.RuleRepository` port (new), `RulesV2.EventLog` port (new), in-memory adapters for both, Mongo adapters for both, shared contract suites

## Goal

Land the persistence layer for the v2 rule engine in its own greenfield Mongo collection so the v1 and v2 engines coexist behind the feature flag (per ADR 0016, #387).
...
Both the Mongo and in-memory adapters run through one shared contract suite so the ports-and-adapters discipline is enforced (per ADR 0001).
...
The critical failure mode (writing to an unknown rule/symbol id): v1's contract behavior is that `$push` against a missing doc is a no-op (`matchedCount === 0`); v2 mirrors that — reads against the absent id return `[]`. The contract asserts the empty-array reads cover that.
