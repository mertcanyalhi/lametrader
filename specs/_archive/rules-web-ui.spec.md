...
The web UI needs a v2-shaped rule editor — feature-flag-gated so it doesn't surface to users until the hard cutover — that walks one user through trigger / scope / condition / action selection end-to-end and round-trips a `Rule` through `/rules`.
...
- **`Rule` (v2)** — the persisted shape exposed at `/rules*`: `{ id, profileId, name, scope, trigger, condition, expiration, actions, enabled, order, createdAt, updatedAt }`.
...
- **Feature flag (historical, removed in #397 cutover)** — `localStorage.rulesEnabled === 'true'` (also unlockable via `?rules=1` URL param so the flag can be flipped from a browser without typing into devtools). Default off — the route + sidebar entry are absent when the flag is off.
...
- A `useRulesEnabled` hook reads the feature flag (URL param `rules=1` wins, else `localStorage.rulesEnabled === 'true'`, else `false`). (Removed in the #397 cutover.)
- The `/rules` route is mounted only when the flag is enabled.
- The sidebar's "Rules v2" entry is visible only when the flag is enabled.
- `lib/hooks/rules.ts` exposes `useRules(filters?)` → `GET /rules?profileId=&symbolId=&enabled=`, `useCreateRule` → `POST /rules`, `usePatchRule` → `PATCH /rules/:id`, `useDeleteRule` → `DELETE /rules/:id`.
...
- The `OperandPicker` renders one option per `OperandKind`, labels `Price` (not `Current`), and updates the operand on selection.
...
- The `OperatorPicker` filters its options to operators whose family is legal for the resolved LHS valueType (numeric LHS hides State operators; bool LHS only shows State family).
- The `ConditionTreeEditor` renders the bool-shortcut layout (operator + RHS hidden) when the LHS resolves to a Bool-typed operand; the saved leaf carries `Equals` against `Literal(true)`.
...
- The `TriggerPicker` exposes all six `TriggerKind` options; selecting `OncePerBar` / `OncePerBarOpen` / `OncePerBarClose` reveals a `period` field; `OncePerInterval` reveals a `intervalMs` field.
- The `ScopePicker` exposes `Symbol`, `Symbols(list)`, `AllSymbols`; `Symbol` reveals a single watched-symbol picker; `Symbols(list)` reveals a multi-select.
...
- The `RuleEditorDialog` round-trips a v2 rule end-to-end: a `useCreateRule` mutation fires with the form's full payload on submit, and a 400 with `fields[]` populates inline field-level error messages (one per offending path).
...
- The same e2e asserts a flag-off bundle does not surface the `/rules` route at the URL root (the flag's presence is enough — the e2e doesn't drive a browser).
