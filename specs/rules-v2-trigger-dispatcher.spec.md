# Spec: rules-v2 trigger dispatcher

- Status: draft
- Touches: `@lametrader/core` — new `RulesV2.RuleRepository` port shape; `@lametrader/engine` — new `rules-v2/dispatch/` module (`TriggerDispatcher`, `IntervalScheduler`, condition-tree walker, slot-reference predicate).

## Goal

The dispatch layer that takes each `RulesV2.RuleEvent` (#388) and routes it to matching rules by trigger granularity, evaluates their condition trees (`evaluateLeaf` from #390 + AND/OR composition), and runs each trigger's firing gate.
...
- Bridges that translate Polling/Quote events to `RulesV2.RuleEvent` — #392's surface.
