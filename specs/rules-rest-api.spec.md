# Spec: rules REST API (`/rules*`)

- Status: draft
- Touches: `.RuleRepository` / `.EventLog` ports (driven); v2 application use-case `RuleService` (new); v2 wire helper `wireRuleEngine` (new); driving HTTP adapter `rulesV2Controller` (new), `app.ts` wiring + global error handler.

## Goal

Expose v2 rules over a REST surface mounted at `/rules` so the v2 engine is reachable from the API.
Adds a v2 application service (`RuleService`) over the existing v2 ports, a `wireRuleEngine` analogue that plugs the v2 orchestrator into the live event chain alongside v1, and a Fastify controller with TypeBox-validated payloads.
...
- [ ] `POST /rules` with a valid body returns `201` and the assembled rule.
- [ ] `POST /rules` with a body that fails schema (e.g. missing `scope`, wrong action `kind`, bad `expiration` shape) returns `400` with `{ error, fields: [{ path, message }, ...] }` where every offending field is listed.
- [ ] `POST /rules` with a tick-cadence trigger on an unwatched symbol returns `400` with one `fields[]` entry whose `path` points at `scope.symbolId` (or `scope.symbolIds`).
- [ ] `GET /rules` returns `200` and the rule list; `?profileId` / `?symbolId` / `?enabled` filter it.
- [ ] `GET /rules/:id` returns `200` and the rule when present, `404` `{ error }` when not.
- [ ] `PATCH /rules/:id` with a valid partial body returns `200` and the updated rule; invalid → `400`; missing → `404`.
- [ ] `DELETE /rules/:id` returns `204` when present, `404` when not.
- [ ] `GET /rules/:id/events` returns `200` and the rule's events log newest-first; supports `?limit` (default 50, max 500) and `?before` (epoch-ms cursor); `404` when the rule is missing.
...
- [ ] `wireRuleEngine(deps)` constructs the v2 orchestrator over the v2 repos + event log + ActionRunner + TriggerDispatcher + bridges (`tickBridge`, `barBridge`, `indicatorBridge`) plus a `drain()` per the v1 analogue.
- [ ] `connectServices()` constructs `MongoRuleRepository` (v2) + `MongoEventLog` (v2) + `RuleService` and exposes them on the returned bundle so the API layer can register the v2 controller.
...
3. `POST /rules` with a tick-cadence `EveryTime` rule scoped to that symbol with a `Price > 100` condition and a `SetSymbolState` action — expect `201`.
4. Drive a `TickEvent` (price 101) through the v2 quote bridge.
5. `GET /rules/:id/events` returns one `Fired` entry; `GET /v2/symbols/:id/rule-events` mirrors it.

The one critical failure mode the e2e covers:

- `POST /rules` with a tick-cadence trigger on an unwatched symbol rejects `400` with `fields: [{ path: 'scope.symbolId', ... }]`; the rule is NOT persisted (a follow-up `GET /rules` does not return it).

## Out of scope

- A `/rules/order` reorder endpoint (the issue does not list one; defer until UI surfaces a need).
...
- OHLCV / indicator-series resolution in `LiveEvaluationLookups` beyond the per-symbol cache the bridges seed; the spec for "Price > 100" only needs the tick price, which the orchestrator reads off the event.
