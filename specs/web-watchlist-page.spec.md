...
- [ ] The edit dialog offers and pre-selects only periods still enabled in config, so a period the symbol watches but config no longer enables is dropped on save (not re-sent and rejected).
