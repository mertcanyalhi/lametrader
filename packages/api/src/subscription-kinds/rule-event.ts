 ...
 * mirrors {@link candleSubscriptionKind}'s shape.
 *
 * Sync acquire (no upstream service call); the hub subscription is the only
 * resource held, released on unsubscribe / cleanup. Inbound frames carry both
 * `symbolId` and `entry` so the web client can route by key without re-deriving
 * it from the entry.
 */
