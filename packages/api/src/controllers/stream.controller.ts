 ...
 * - rule-event subscriptions via `{ action: 'subscribe-rule-event', id }` — each {@link RuleEventEntry} mirrored to that symbol's events log arrives as a `{ symbolId, entry }` frame. The client unsubscribes via `{ action: 'unsubscribe-rule-event', id }`.
 *
 ...
 *
 * The engine has no WebSocket dependency (ADR-0005); the polling loop feeds the candle hub via `onCandle`, the indicator stream service via its own `onState`, and the rule-event hub via the event-log's `onAppend`.
 *
