...
import type { RuleServiceV2, RuleV2CreateInput } from '@lametrader/engine';
...
/**
 * Register the `/v2/rules*` resource against a {@link RuleServiceV2}.
 *
 ...
 */
export function rulesV2Controller(service: RuleServiceV2) {
  return async (instance: FastifyInstance): Promise<void> => {
    ...
    app.post(
      ...
      async (request, reply) => {
        const created = await service.create(request.body as RuleV2CreateInput);
    ...
    app.patch(
      ...
      async (request) =>
        service.patch(request.params.id, request.body as Partial<RuleV2CreateInput>),
    );
    ...
    app.get(
      '/symbols/:id/rule-events',
      {
        schema: {
          tags: ['rules-v2'],
          summary: 'Read one symbol mirrored v2 rule events log (newest-first)',
