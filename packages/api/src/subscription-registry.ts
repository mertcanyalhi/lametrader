...
/** WebSocket surface the registry needs — narrows the standard `WebSocket` interface. */
...
/**
 * Lifecycle owner for the `/stream` route's multiplexed subscriptions.
 *
 ...
 * array. The controller stays a thin parse-and-dispatch shim.
 */
export class SubscriptionRegistry {
   ...
   */
  attach(socket: SocketLike, log: FastifyBaseLogger): SocketSubscriptions {
 ...
 * single cleanup loop — collapsing today's three near-identical handler
 * triplets in `stream.controller.ts` into one shared lifecycle.
 */
