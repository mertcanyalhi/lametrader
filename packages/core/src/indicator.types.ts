...
/**
 * One live indicator-state update pushed to a subscriber as a candle arrives.
 *
 * `subscriptionId` is generated by the engine at subscribe time so a single client socket can multiplex many subscriptions (each with its own indicator + inputs).
 *
 * `state` is the latest point only — the row at the just-arrived candle's `time`.
 *
 * `final` mirrors the candle's `final` flag: `true` once the bar has closed; `false` while the bar is still forming and the state may update on later polls.
 */
export interface IndicatorStateEvent {
...
/**
 * A transport-agnostic sink the indicator-streaming use-case emits each {@link IndicatorStateEvent} to.
 *
 * Driving adapters render it their own way (the API fans it to WebSocket subscribers); the engine knows nothing about delivery (see ADR-0005).
 */
export type IndicatorStateListener = (event: IndicatorStateEvent) => void;
