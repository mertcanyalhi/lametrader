 ...
 * `listEnabledForSymbol` filter the orchestrator (#393) hits on every inbound
 * event.
 *
 ...
 * feature flag without schema entanglement. #394 lands the Mongo adapter and
 * the contract suite.
 */
export interface RuleRepository {
  ...
  /** Delete `id` from the store. Idempotent — unknown ids are a no-op. */
   ...
   * - With `profileId` set: filters by `rule.profileId === profileId` (used to
   *   scope cascade events to same-profile rules per #281).
   */
