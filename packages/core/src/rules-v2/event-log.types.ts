...
export interface EventLog {
   ...
   * stamped `entry` (so `firedAt` is set) and a discriminated `target`.
   * Each fire mirrors to two appends (rule + symbol), so the listener is
   * invoked twice per fire with the same entry and different `target.kind` —
   * by design; callers filter to the side they care about.
   */
