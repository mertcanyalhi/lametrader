...
interface BaseRuleEventEntry {
   ...
   * drove the fire (e.g. a candle bar's open for OHLCV events, a quote's
   * tick time).
   */
  ts: number;
  /**
 ...
 * are suppressed.
 * Emitted at most once per (rule, symbol).
 */
 ...
 *
 * Tagged union over {@link RuleEventType}; mirrored onto the affected
 * symbol's embedded events array.
 */
