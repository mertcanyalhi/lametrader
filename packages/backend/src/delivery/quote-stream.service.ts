 ...
 *
 * Lives in-process — the subscription map is non-durable (same boundary as the
 * indicator stream surface).
 */
 ...
 *
 * Transport-agnostic — emission happens via the `onQuote` callback; the `/stream` gateway renders it to WebSocket frames.
 *
 * Relocated as-is from the engine's `QuoteStreamService`; the `/stream` gateway
 * drives its `subscribe` / `unsubscribe`, and the live `handleCandle` feed stays
 * dormant until the polling cutover (#490) fans candles into it.
 */
