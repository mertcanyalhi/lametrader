...
/** A fake socket whose `send` records every frame. */
...
/** A no-op {@link StreamLog} for tests. */
function noopLog(): StreamLog {
  return { info: () => {}, warn: () => {} };
}
...
describe('SubscriptionRegistry — dispatch', () => {
  it('routes a subscribe message to the matching kind by action string', async () => {
    const acquire = jest.fn((input: { id: string }) => ({ key: input.id }));
