 ...
 *
 * The `valueType` is the variant of the latest observed `StateSet` for this key
 * under the symbol, used by the chart layer to decide between line (numeric) and
 * marker (bool/string) rendering.
 */
 ...
 *
 * Each entry is sourced from a `StateSet` or `StateRemoved` rule event on the
 * symbol's mirrored events log; `value === null` marks a removal.
 */
