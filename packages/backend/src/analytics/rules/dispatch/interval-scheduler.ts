 ...
 */
export type IntervalEmit = (event: TimerEvent, ruleId: string) => void;

/**
 * Wall-clock scheduler arming one chained `setTimeout` per
 * `OncePerInterval`-triggered rule.
 *
 * Each rule's chain emits a `TimerEvent` whose `ts` is the boundary
...
export class IntervalScheduler {
   ...
   * timer keeps running).
   */
  start(ruleId: string, trigger: OncePerIntervalTrigger): void {
  ...
  private scheduleNext(ruleId: string, intervalMs: number): void {
    const handle = setTimeout(() => {
      this.emit({ kind: EvaluationTriggerKind.Timer, ts: this.now() }, ruleId);
