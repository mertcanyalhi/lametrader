 ...
 * behaviour-identical to the old native-driver `MongoStateRepository`.
 *
 * **Partitioned by `profileId`** (#281): every read/write takes a profile id;
 * the contract asserts that two profileIds do NOT see each other's writes even
 * when `(symbolId, key)` matches.
 *
