 ...
 *
 * Real (not a test double): backs the unit tier and doubles as the fake
 * substituted for the Mongoose adapter under a Nest DI override in tests.
 *
 * Partitioned by `profileId` (#281): the outer map's key is `profileId`, so
 * two profiles operating on the same `(symbolId, key)` see independent values.
 */
