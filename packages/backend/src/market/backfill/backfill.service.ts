 ...
 * symbol+period, plus reading stored candles back one keyset page at a time.
 *
 ...
 * via an `onProgress` callback so transports (the API's per-job WebSocket) render
 * it without the application knowing about them (ADR-0005).
 *
 * Relocated verbatim from the engine; the concrete adapters are injected by the
