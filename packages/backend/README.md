...
- `curl 'http://localhost:3000/symbols/crypto:BTCUSDT/indicators/supertrend?period=1h&atrPeriod=10&multiplier=3&atrMethod=rma'`
...
The `period` query param is reserved for the candle sampling period, so Supertrend's ATR length rides as `atrPeriod` (not `period`).
