import { type EventLog, RuleEventType, StateScope } from '@lametrader/core';
import type {
  StateHistoryEntry,
  StateHistoryWindow,
  StateKeyDescriptor,
...
/**
 * Read use-case for chart-side state overlays.
 *
 * Sources state history from the existing {@link EventLog} (no new
 * persistence): `StateSet` and `StateRemoved` entries on the symbol's
 * mirrored events array become the time-series points.
 *
 * State is partitioned per-profile at the {@link StateRepository} layer, but
 ...
 * Multi-profile filtering would require adding `profileId` to the persisted
 * event shape — a schema bump deferred to a follow-up.
 */
export class StateHistoryService {
  /**
   * @param eventLog - the mirrored rule-event log to read from.
   */
  constructor(private readonly eventLog: EventLog) {}

  /**
   * Distinct `(key, valueType)` pairs from the symbol's symbol-scoped
   * `StateSet` entries.
   *
