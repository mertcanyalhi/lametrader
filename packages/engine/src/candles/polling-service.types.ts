 ...
 * Driving adapters render it their own way (the API fans it to WebSocket
 * subscribers); the application knows nothing about delivery (see ADR-0005).
 */
export type CandleListener = (event: CandleEvent) => void;

/**
 * Options for {@link PollingService}: the event sink, the per-period poll
 * cadence, and injectable clock/jitter sources (defaulted for production).
 */
