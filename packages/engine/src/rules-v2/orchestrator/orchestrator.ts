import { RulesV2, type StateRepository, type WatchlistRepository } from '@lametrader/core';
 ...
 * 3. Cascades any {@link RulesV2.SymbolStateChangedEvent} /
 *    {@link RulesV2.GlobalStateChangedEvent} emitted during the tick back
...
export class RuleOrchestrator {
  ...
  async process(initialEvent: RulesV2.EvaluationTriggerEvent): Promise<void> {
    ...
    try {
      const queue: Array<{ event: RulesV2.EvaluationTriggerEvent; cascadeDepth: number }> = [
  ...
  private async runOneFire(
    fire: { ruleId: string; firingSymbolId: string; event: RulesV2.RuleEvent },
    inboundEvent: RulesV2.EvaluationTriggerEvent,
  ): Promise<void> {
