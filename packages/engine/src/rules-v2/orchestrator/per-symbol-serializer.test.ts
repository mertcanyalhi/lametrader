...
function tick(symbolId: string, ts: number): RulesV2.EvaluationTriggerEvent {
  return {
...
function timer(ts: number): RulesV2.EvaluationTriggerEvent {
  return { kind: RulesV2.EvaluationTriggerKind.Timer, ts };
}
...
describe('createPerSymbolSerializer', () => {
  it('runs successive events for the same symbol sequentially while symbol-less events share one global chain, and drain() resolves once every chain settles', async () => {
    ...
    const serializer = createPerSymbolSerializer<RulesV2.EvaluationTriggerEvent>(async (event) => {
      const label =
