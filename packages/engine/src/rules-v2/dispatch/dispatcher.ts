 ...
 * orchestrator owns the {@link RulesV2.RuleEventContext} payload (it has the
...
export interface FireRecord {
  ...
  event: RulesV2.RuleEvent;
}

/**
 * The trigger dispatcher — turns one inbound {@link RulesV2.RuleEvent} into
...
export class TriggerDispatcher {
   ...
   */
  async dispatch(event: RulesV2.RuleEvent, options: DispatchOptions = {}): Promise<FireRecord[]> {
    ...
    if (event.kind === RulesV2.EvaluationTriggerKind.BarOpened) {
    ...
    for (const { rule, firingSymbolId } of candidates) {
      ...
      if (rule.trigger.kind === RulesV2.TriggerKind.Once) {
  ...
  private async candidates(
  ...
  ): Promise<Array<{ rule: RulesV2.Rule; firingSymbolId: string }>> {
    const profileId =
    ...
    const out: Array<{ rule: RulesV2.Rule; firingSymbolId: string }> = [];
  ...
  private recordFire(
    ...
    if (rule.trigger.kind === RulesV2.TriggerKind.OncePerBar) {
      const key = latchKey(rule.id, firingSymbolId, rule.trigger.period);
      this.oncePerBarLatch.add(key);
    } else if (rule.trigger.kind === RulesV2.TriggerKind.OncePerInterval) {
      const key = latchKey(rule.id, firingSymbolId);
   ...
   */
  private rearmOncePerBarLatches(event: RulesV2.BarOpenedEvent): void {
  ...
  ): Promise<RulesV2.Rule[]> {
    const byId = new Map<string, RulesV2.Rule>();
...
function symbolIdOf(event: RulesV2.EvaluationTriggerEvent): string | null {
  if (
    event.kind === RulesV2.EvaluationTriggerKind.Tick ||
...
function isEvaluationTriggerEvent(
  event: RulesV2.RuleEvent,
): event is RulesV2.EvaluationTriggerEvent {
  return (
    event.kind === RulesV2.EvaluationTriggerKind.Tick ||
    event.kind === RulesV2.EvaluationTriggerKind.BarOpened ||
    event.kind === RulesV2.EvaluationTriggerKind.BarClosed ||
    event.kind === RulesV2.EvaluationTriggerKind.Timer ||
    event.kind === RulesV2.EvaluationTriggerKind.SymbolStateChanged ||
    event.kind === RulesV2.EvaluationTriggerKind.GlobalStateChanged ||
    event.kind === RulesV2.EvaluationTriggerKind.IndicatorChanged
  );
