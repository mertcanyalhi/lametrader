...
const TRIGGER_60S: RulesV2.OncePerIntervalTrigger = {
  kind: RulesV2.TriggerKind.OncePerInterval,
  intervalMs: 60_000,
};

const TRIGGER_30S: RulesV2.OncePerIntervalTrigger = {
  kind: RulesV2.TriggerKind.OncePerInterval,
  intervalMs: 30_000,
};
...
describe('IntervalScheduler', () => {
  ...
  it('emits a Timer event at the rule trigger intervalMs cadence', () => {
    const emitted: RulesV2.TimerEvent[] = [];
    const scheduler = new IntervalScheduler((event) => emitted.push(event));
    scheduler.start('rule-1', TRIGGER_60S);
    vi.advanceTimersByTime(60_000);
    expect(emitted).toEqual([{ kind: RulesV2.EvaluationTriggerKind.Timer, ts: 60_000 }]);
  });
