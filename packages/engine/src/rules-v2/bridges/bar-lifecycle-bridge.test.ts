import { type EquityCandle, Period, RulesV2, SymbolType } from '@lametrader/core';
...
describe('BarLifecycleBridge', () => {
  ...
  it('emits nothing on a re-poll of the same forming bar (same candle.time, final = false)', () => {
    const events: RulesV2.EvaluationTriggerEvent[] = [];
    const bridge = new BarLifecycleBridge((event) => events.push(event));
    bridge.handleCandle(
  ...
  it('emits BarClosed on the first observation with final = true for a (symbolId, period, ts) and dedupes subsequent final observations on the same ts', () => {
    const events: RulesV2.EvaluationTriggerEvent[] = [];
    const bridge = new BarLifecycleBridge((event) => events.push(event));
    bridge.handleCandle(
      candleEvent({ id: 'AAPL', period: Period.OneMinute, time: 60_000, final: false }),
