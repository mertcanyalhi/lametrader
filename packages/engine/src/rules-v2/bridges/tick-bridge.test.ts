...
describe('TickBridge', () => {
  ...
  it('emits one TickEvent per inbound quote (no dedup) for a burst on the same symbol', () => {
    const events: RulesV2.EvaluationTriggerEvent[] = [];
    const bridge = new TickBridge((event) => events.push(event));
    bridge.handleQuote(quoteEvent({ id: 'BTC', price: 100, time: 1_000 }));
    bridge.handleQuote(quoteEvent({ id: 'BTC', price: 100, time: 1_001 }));
    bridge.handleQuote(quoteEvent({ id: 'BTC', price: 101, time: 1_002 }));
    expect(events).toEqual([
      { kind: RulesV2.EvaluationTriggerKind.Tick, ts: 1_000, symbolId: 'BTC', price: 100 },
      { kind: RulesV2.EvaluationTriggerKind.Tick, ts: 1_001, symbolId: 'BTC', price: 100 },
      { kind: RulesV2.EvaluationTriggerKind.Tick, ts: 1_002, symbolId: 'BTC', price: 101 },
    ]);
  });
