...
/**
 * Build a fresh {@link RulesV2.EventLog} under test, stamped with a fixed
 * `firedAt` clock so full-payload assertions stay deterministic.
 */
export interface EventLogV2Factory {
  /** The event-log under test, freshly empty. */
  log: RulesV2.EventLog;
...
/**
 * The shared behavioural contract every {@link RulesV2.EventLog} must
 * satisfy.
 *
...
export function runEventLogContract(
  make: () => EventLogV2Factory | Promise<EventLogV2Factory>,
): void {
  /** A minimal `Fired` umbrella entry. */
  function firedEntry(): RulesV2.FiredRuleEvent {
    return {
  ...
  function notificationEntry(): RulesV2.NotificationSentRuleEvent {
    return {
  ...
  function stateSetEntry(): RulesV2.StateSetRuleEvent {
    return {
  ...
  function stateRemovedEntry(): RulesV2.StateRemovedRuleEvent {
    return {
  ...
  function errorEntry(): RulesV2.ErrorRuleEvent {
    return {
  ...
  function cycleOverflowEntry(): RulesV2.CycleOverflowRuleEvent {
    return {
  ...
  it('preserves a caller-supplied firedAt (does not re-stamp)', async () => {
    const { log } = await make();
    const entry: RulesV2.NotificationSentRuleEvent = {
      ...notificationEntry(),
      firedAt: 1234,
    };
    await log.appendRuleEvent('rule-1', entry);
  ...
  it('onAppend unsubscribe stops further notifications', async () => {
    const { log } = await make();
    const observed: RulesV2.RuleEventEntry[] = [];
