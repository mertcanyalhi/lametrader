...
const fakeCtx = (leftSeries: SeriesView): EvaluationContext => ({
  ...
  resolveSeries: (operand) =>
    operand.kind === RulesV2.OperandKind.Price ? leftSeries : EMPTY_SERIES,
});
...
describe('evaluateMoving', () => {
  ...
  it('returns true for MovingDownPercent when (past - current) / past * 100 >= threshold, and false when past is 0 (no divide-by-zero)', () => {
    ...
    expect({
      down: evaluateMoving(movingLeaf(RulesV2.MovingOperator.MovingDownPercent, 10, 1), downCtx),
      zeroPast: evaluateMoving(
        movingLeaf(RulesV2.MovingOperator.MovingDownPercent, 10, 1),
        zeroPastCtx,
      ),
  ...
  it('returns false when the series has fewer than lookbackBars + 1 samples', () => {
    ...
    expect({
      short: evaluateMoving(movingLeaf(RulesV2.MovingOperator.MovingUp, 5, 3), shortCtx),
      empty: evaluateMoving(movingLeaf(RulesV2.MovingOperator.MovingUp, 5, 3), emptyCtx),
