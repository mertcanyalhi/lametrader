...
const fakeCtx = (overrides: {
  ...
  resolveLatest: (operand) => {
    ...
    if (overrides.rightLatest && operand.kind !== RulesV2.OperandKind.Price)
      return overrides.rightLatest;
    return null;
  },
  resolvePrev: () => null,
  resolveSeries: (operand) => {
    if (operand.kind === RulesV2.OperandKind.Price) return overrides.priceSeries ?? EMPTY_SERIES;
...
describe('evaluateCrossing', () => {
  ...
  it('produces the same verdict for cross-frequency (rare vs frequent right updates) — asOf resampling decouples cadence from result', () => {
    ...
    const leaf = priceCrossingIndicator(RulesV2.CrossingOperator.CrossingUp);
    expect({
