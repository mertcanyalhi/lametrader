...
/**
 * Shape of a `rules_v2` document with its embedded v2 events array — the
 * only fields {@link MongoEventLog} reads or writes on the rule side.
 */
 ...
 * watchlist document is untouched (per #394 — v1 and v2 coexist behind the
 * feature flag).
 */
 ...
 * is `$push`-appended onto the parent rule's `rules_v2.{ruleId}.events`
 * array AND the affected symbol's `watchlist.{symbolId}.events_v2` array.
 *
 * The two-write fan-out is not atomic — an interleaved failure may leave one
 * side missing an entry.
 * Acceptable for an events log (occasional gaps don't change correctness)
 * and matches the in-memory adapter's contract.
 *
 * The symbol-side write lands on the existing `watchlist` collection but
 * under the new `events_v2` field, so v1's `events` array is never touched
 * and the two engines coexist behind the feature flag.
 */
