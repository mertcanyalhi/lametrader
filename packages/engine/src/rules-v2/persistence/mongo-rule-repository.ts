 ...
 *
 * `save` uses `$set` per field (preserving the event-log's embedded `events`
 * mirror) so a `save` after some events have been appended doesn't wipe the
 * log.
 *
 * Profile-enabled (`profile.enabled` kill-switch) filtering is deferred
 * until profiles-v2 lands; mirrors the in-memory v2 adapter and matches the
 * issue #394 scope.
 * `profileId` filtering is honoured.
 */
export class MongoRuleRepository implements RulesV2.RuleRepository {
  ...
  /**
   * Create the orchestrator hot-path indexes.
   * Idempotent.
   *
   * The orchestrator hits `listEnabledForSymbol(symbolId, profileId?)` on
   * every inbound event, so the indexes prune by `enabled` + scope-id +
   * `profileId` before the in-memory scope match:
   *
   * - `{ profileId: 1, 'scope.symbolId': 1, enabled: 1 }` — Symbol-scoped
   *   reads filtered by profile.
   * - `{ enabled: 1, 'scope.symbolIds': 1 }` — Symbols-scoped reads
   *   (multikey over the symbolIds array).
