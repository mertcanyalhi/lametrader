...
const { BarLifecycleBridge, IndicatorCascadeBridge, StateCascadeBridge, TickBridge } =
  EngineRulesV2;
...
describe('rules-v2 bridges (e2e)', () => {
  it('drives real PollingService + QuoteStreamService + IndicatorService + StateRepository through the v2 bridges and emits the expected EvaluationTriggerEvent stream', async () => {
    const emitted: RulesV2.EvaluationTriggerEvent[] = [];
    const sink = (event: RulesV2.EvaluationTriggerEvent): void => {
      emitted.push(event);
    };
    ...
    const barOpenedForPolledOnly = emitted.filter(
      (e) =>
        e.kind === RulesV2.EvaluationTriggerKind.BarOpened && e.symbolId === SYMBOL_POLLED_ONLY,
    );
    const tickForPolledOnly = emitted.filter(
      (e) => e.kind === RulesV2.EvaluationTriggerKind.Tick && e.symbolId === SYMBOL_POLLED_ONLY,
    );
    const tickForQuoted = emitted.filter(
      (e) => e.kind === RulesV2.EvaluationTriggerKind.Tick && e.symbolId === SYMBOL_QUOTED,
    );
    const barOpenedForQuoted = emitted.filter(
      (e) => e.kind === RulesV2.EvaluationTriggerKind.BarOpened && e.symbolId === SYMBOL_QUOTED,
    );
    const cascade = emitted.filter(
      (e) =>
        e.kind === RulesV2.EvaluationTriggerKind.SymbolStateChanged && e.symbolId === SYMBOL_QUOTED,
    );
    const indicatorChanged = emitted.filter(
      (e) => e.kind === RulesV2.EvaluationTriggerKind.IndicatorChanged,
    );
