...
const { buildEvaluationContext, IndicatorSeriesStore, prewarmBarSeries, TickRing } = EngineRulesV2;
type BarAxis = EngineRulesV2.BarAxis;
...
describe('rules-v2 series store + EvaluationContext (e2e)', () => {
  it('drives the full stack — seeded candles, indicator warmup, fresh bar + ticks — and resolves every operand kind end-to-end', async () => {
    ...
    expect({
      price: ctx.resolveLatest({ kind: RulesV2.OperandKind.Price }),
      close: ctx.resolveLatest({ kind: RulesV2.OperandKind.Close }),
      high: ctx.resolveLatest({ kind: RulesV2.OperandKind.High }),
    ...
    const priceSeries = ctx.resolveSeries({ kind: RulesV2.OperandKind.Price });
    expect({
  ...
  it('returns an empty series for an unwatched symbol — operators see length 0 rather than crashing', async () => {
    ...
    const priceSeries = ctx.resolveSeries({ kind: RulesV2.OperandKind.Price });
    const closeLatest = ctx.resolveLatest({ kind: RulesV2.OperandKind.Close });
