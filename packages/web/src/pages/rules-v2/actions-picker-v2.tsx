...
/**
 * Human-readable label for each {@link RulesV2.ActionKind}.
 *
 * Used by the picker's kind dropdown and the action card header.
 */
export const ACTION_KIND_LABELS: Readonly<Record<RulesV2.ActionKind, string>> = {
  [RulesV2.ActionKind.Notification]: 'Send notification',
  [RulesV2.ActionKind.SetSymbolState]: 'Set symbol state',
  [RulesV2.ActionKind.RemoveSymbolState]: 'Remove symbol state',
  [RulesV2.ActionKind.SetGlobalState]: 'Set global state',
  [RulesV2.ActionKind.RemoveGlobalState]: 'Remove global state',
};
 ...
 */
export function ActionsPickerV2({
  value,
  onChange,
}: {
    ...
    <Card variant="surface">
      <Flex direction="column" gap="2">
        <Flex align="center" gap="2" justify="between">
          <Select.Root
            value={value.kind}
            onValueChange={(next) => onChange(actionFromKind(next as RulesV2.ActionKind, value))}
          >
            <Select.Trigger aria-label="Action kind" />
            <Select.Content>
              {Object.values(RulesV2.ActionKind).map((kind) => (
...
function ActionBody({
  ...
  switch (value.kind) {
    case RulesV2.ActionKind.Notification:
...
function NotificationBody({
  ...
  value: RulesV2.NotificationAction;
  onChange: (next: RulesV2.Action) => void;
}): ReactNode {
...
function SetStateBody({
  ...
  value: RulesV2.SetSymbolStateAction | RulesV2.SetGlobalStateAction;
  onChange: (next: RulesV2.Action) => void;
}): ReactNode {
  return (
    <Flex direction="column" gap="2">
...
function RemoveStateBody({
  ...
  value: RulesV2.RemoveSymbolStateAction | RulesV2.RemoveGlobalStateAction;
  onChange: (next: RulesV2.Action) => void;
}): ReactNode {
  return (
    <TextField.Root
 ...
 */
function actionFromKind(kind: RulesV2.ActionKind, prev: RulesV2.Action): RulesV2.Action {
  switch (kind) {
    case RulesV2.ActionKind.Notification:
      return {
...
function defaultAction(): RulesV2.Action {
  return {
    kind: RulesV2.ActionKind.Notification,
    channel: RulesV2.NotificationChannel.Telegram,
    destinationName: '',
