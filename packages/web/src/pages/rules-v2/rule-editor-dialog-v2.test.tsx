...
import { RuleEditorDialogV2 } from './rule-editor-dialog-v2';

/** A wrapper that wires the dialog with a fresh TanStack client + Theme. */
function Harness({ initial }: { initial: RulesV2.Rule }): ReactNode {
  const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
  return (
    <QueryClientProvider client={client}>
      <Theme>
        <RuleEditorDialogV2 open={true} onOpenChange={() => {}} mode="create" initial={initial} />
      </Theme>
    </QueryClientProvider>
  );
}
...
describe('RuleEditorDialogV2', () => {
  ...
  it('shows the v2 create-mode title with the v2 suffix and a Create submit button', async () => {
    render(<Harness initial={makeDraftRuleV2({ profileId: 'p1' })} />);
    await waitFor(() => {
      expect(screen.getByText('New rule (v2)')).toBeInTheDocument();
    });
    expect(screen.getByRole('button', { name: 'Create' })).toBeInTheDocument();
  });

  it('POSTs to /api/v2/rules with the assembled body on submit and surfaces the API field error inline', async () => {
    ...
    const seed = makeDraftRuleV2({ profileId: 'p1', symbolId: 'crypto:BTCUSDT' });
    ...
    // Intercept the POST /v2/rules call so we can return a field-level error;
    // every auxiliary read falls through to a sensible empty default per URL.
    fetchSpy.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = typeof input === 'string' ? input : input.toString();
      if (url === '/api/v2/rules' && init?.method === 'POST') {
        return new Response(
          JSON.stringify({
    ...
    const postCall = fetchSpy.mock.calls.find(
      ([url, init]) =>
        (typeof url === 'string' ? url : url.toString()) === '/api/v2/rules' &&
