import type { Candle, Period, SymbolQuoteEvent } from '@lametrader/core';
 ...
 * {@link SymbolQuoteEvent}s. Lets `subscribe`/`useStreamSubscription` be generic
 * over the kind while staying fully typed.
 */
...
export interface StreamClient {
   ...
   * The first listener for a `(kind, id)` opens/uses the shared socket and sends
   ...
   */
  subscribe<K extends StreamKind>(kind: K, id: string, listener: StreamListener<K>): Unsubscribe;
  /**
