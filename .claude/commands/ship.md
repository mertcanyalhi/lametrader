...
1. Run `npm run check:full`. If anything fails, STOP and report — do not commit.
2. Verify the Definition of Done (CLAUDE.md): a spec exists, unit + e2e tests derive
   from it and pass, nothing is `.skip`-ped, and an ADR was written if a non-obvious
   decision was made.
